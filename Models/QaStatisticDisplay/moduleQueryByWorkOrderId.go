...
func GetWorkOrderIds(queryCondition *QueryCondition) (qaWorkOrderIdList []QaWorkOrderId, err error) {
	sqlStr := `with TRX as (SELECT distinct a.MANUFACTURE_GROUP,d.LOT_TYPE,
				...
				FROM (select t.*,dense_rank()over(partition by T.PARTNUMBER,T.BOSA_SN order by T.MODIFYDATE DESC)LA from superxon.autodt_tracking t)C where C.LA=1) a,
				...
				where b.sn=a.bosa_sn and a.partnumber =b.pn and d.pch_tc=a.manufacture_group and b.pn=d.partnumber
				and ( b.pn LIKE  '` + queryCondition.Pn + `%' AND D.LOT_TYPE LIKE '` + queryCondition.WorkOrderType + `%')
				...
				select e.* from
				(select distinct h.manufacture_group as 工单号,h.PN as PN,h.LOT_TYPE as 工单类型
				from TRX h where h.rr=1)e
				join superxon.sgd_scdd_trx t on e.工单号=t.pch_tc
				order by e.工单号,e.pn ASC`
...
func GetWorkOrderYieldsByWorkOrderId(queryCondition *QueryCondition) (workOrderYieldList []WorkOrderYieldByWorkOrderId, err error) {
	sqlStr := `with TRX as (SELECT distinct a.MANUFACTURE_GROUP,b.*,
				...
				from superxon.autodt_tracking t)aa where aa.LA=1) a,superxon.workstage c,superxon.sgd_scdd_trx d
