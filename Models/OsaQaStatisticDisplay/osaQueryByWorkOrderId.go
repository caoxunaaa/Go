...
func GetQaOsaStatisticInfoListByWorkOrderId(osaQueryCondition *OsaRunDisplay.OsaQueryCondition) (qaOsaStatisticInfoByWorkOrderIdList []QaOsaStatisticInfoByWorkOrderId, err error) {
	sqlStr := `with OSA as (select * from (select s.lotno,t.PN,(case when length(t.pn) <= '14' then 'BOSA' else (substr(T.pn,16,instr(T.pn,'G')))end) as 系列,
				...
				and (  s.lotno = '` + osaQueryCondition.WorkOrderId + `') order by testdate desc)
				union all
				...
				and   (  p.lotno = '` + osaQueryCondition.WorkOrderId + `') order by to_date(o.apd_t_date,'yyyy-mm-dd hh24:mi:ss') desc)
				union all
				...
				where x.pn=y.pn and x.sn=y.sn and  (  y.lotno = '` + osaQueryCondition.WorkOrderId + `') order by to_date(x.rosa_t_date,'yyyy-mm-dd hh24:mi:ss') desc) j
				...
				from superxon.autodt_transmit_autocouple t where (  T.WORKNUM = '` + osaQueryCondition.WorkOrderId + `') order by testtime desc) j join superxon.workstage h on h."processname"=j.TC_FLAG)
				union all
				...
				where ( T.WORKNUM = '` + osaQueryCondition.WorkOrderId + `')and T.SN <> 'NULL' order by testtime desc) j join superxon.workstage h on h."processname"=J.TC_FLAG))
				...
				with OSA as (select * from (select s.lotno,t.PN,(case when length(t.pn) <= '14' then 'BOSA' else (substr(T.pn,16,instr(T.pn,'G')))end) as 系列,
				t.sn,t.errorcode,t.tc_flag,t.testdate as 时间,
				dense_rank()over(partition by t.pn,t.sn,t.tc_flag order by t.testdate asc)rr,
				dense_rank()over(partition by t.pn,t.sn,t.tc_flag order by t.testdate desc)zz,
				h."sequence" as SEQ
